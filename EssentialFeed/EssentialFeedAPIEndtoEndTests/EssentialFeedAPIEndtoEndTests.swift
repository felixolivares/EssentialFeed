...
//
//  Created by José Félix Olivares Estrada on 21/08/24.
//
...
final class EssentialFeedAPIEndtoEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_matchesFixedTestAccountData() {
        ...
        default:
            XCTFail("Expected successfule feed result, got no result instead")
        }
    }
    ...
    func test_endToEndTestServerGETFeedImageDataResult_matchesFixedTestAccountData() {
        ...
        default:
            XCTFail("Expected successful image data resul, got no result instead")
        }
    }
    ...
    private func getFeedResult(file: StaticString = #filePath, line: UInt = #line) -> FeedLoader.Result? {
        ...
        var receivedResut: FeedLoader.Result?
        loader.load { result in
            receivedResut = result
            exp.fulfill()
        }
        wait(for: [exp], timeout: 5.0)
        return receivedResut
    }
